...
// Tối ưu ảnh cho Slider xem trên web (Nhẹ, load nhanh)
const optimizeImg = (url) => url?.includes('cloudinary.com') ? url.replace('/upload/', '/upload/w_1000,c_limit,q_auto,f_auto/') : url;

// Tối ưu ĐẶC BIỆT DÀNH CHO POSTER (Ép đúng kích thước 600x630 để iPhone không bị nghẽn RAM)
...
export default function PropertyDetail() {
  ...
  // KHÓA NÚT TẢI BAN ĐẦU CHỜ NẠP ẢNH
  ...
  useEffect(() => {
    if (property) {
      if (property.images && property.images.length > 0) {
        // TẢI ẢNH ĐÃ ĐƯỢC ÉP KÍCH THƯỚC CHUẨN 600x630 ĐỂ GIẢM DUNG LƯỢNG BASE64 XUỐNG MỨC THẤP NHẤT
  ...
  const handleDownloadPoster = async () => {
    ...
    try {
      ...
      // CHỤP MỒI CHO SAFARI TỈNH NGỦ (DRY RUN)
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 text-gray-800 flex flex-col relative z-0 pb-20 md:pb-0">
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 flex flex-col relative z-0 pb-20 md:pb-0 overflow-x-hidden">
      ...
      {/* TẢI FONT BÊN NGOÀI ĐỂ TRÁNH NGHẼN CỔ CHAI */}
      <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@500;600;700;800;900&display=swap" rel="stylesheet" />

      {/* MÀN HÌNH CHỜ (LOADING) CHUẨN KHI ĐANG VẼ ẢNH */}
      {isGeneratingPoster && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-sm z-[99999] flex flex-col items-center justify-center">
      ...
      {/* TẤM POSTER ẢO ĐƯỢC GIẤU SAU NỀN TRANG WEB BẰNG Z-INDEX ÂM, NẰM TRONG VIEWPORT ĐỂ SAFARI BẮT BUỘC RENDER */}
      <div 
      ...
      >
        ...
        >
          <div style={{ display: 'flex', width: '100%', height: '100%', backgroundColor: '#ffffff', overflow: 'hidden' }}>
             ...
             {/* BÊN TRÁI: DÙNG IMG KHAI BÁO CỨNG KÍCH THƯỚC ĐỂ KHÔNG BỊ TRẮNG ẢNH TRÊN IPHONE */}
             ...
             <div style={{ width: '600px', height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', padding: '50px 40px 40px 40px' }}>
      ...
      <main className="max-w-[1200px] mx-auto px-4 md:px-8 py-8 flex-grow w-full relative z-10">
