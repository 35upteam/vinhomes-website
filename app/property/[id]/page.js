...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    if (property) {
      if (property.images && property.images.length > 0) {
        // ĐÃ SỬA: Thay thế ảnh gốc bằng ảnh đã được giảm kích thước đúng khung 600x630
  ...
  // ĐÃ SỬA: ÉP SAFARI RENDER BẰNG CÁCH CHỤP MỒI 2 NHỊP VÀ HIỂN THỊ MÀN HÌNH CHỜ
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 flex flex-col relative pb-20 md:pb-0 overflow-x-hidden">
      ...
      {/* ĐÃ SỬA: GIẤU POSTER BẰNG OPACITY 1 VÀ Z-INDEX ÂM, NẰM TRONG VIEWPORT ĐỂ SAFARI BẮT BUỘC RENDER */}
      <div 
        className="fixed top-0 left-0 pointer-events-none" 
        style={{ width: '1200px', height: '630px', zIndex: -9999, opacity: isGeneratingPoster ? 1 : 0.01, backgroundColor: '#ffffff', fontFamily: 'sans-serif' }} 
        ref={posterRef}
      >
        <div style={{ display: 'flex', width: '100%', height: '100%', backgroundColor: '#ffffff', overflow: 'hidden' }}>
           
           {/* BÊN TRÁI: KHAI BÁO CỨNG WIDTH/HEIGHT ĐỂ SAFARI KHÔNG TỪ CHỐI RENDER */}
      ...
      {/* TOÀN BỘ GIAO DIỆN WEB BÊN DƯỚI ĐƯỢC GIỮ NGUYÊN 100% */}
      <header className="bg-white sticky top-0 z-50 px-4 md:px-8 py-3 flex justify-between items-center shadow-sm">
